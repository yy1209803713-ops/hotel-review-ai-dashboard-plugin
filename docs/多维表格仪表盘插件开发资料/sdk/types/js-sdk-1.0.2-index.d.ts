...
interface Env {
    product: Product;
    /** 当前环境是否需要切换base，若需要，以workspace为统一入口 */
...
/** [tableId, fieldId, baseToken] */
type FieldContext = [string, string, string | undefined];
...
interface IFieldApplicationModule<V extends ICellTransformVal = any, R extends IOpenCellValue = any, TV extends R | Promise<R> = any> {
    ...
    /**
     * 分页获取整列 cellValue。基于getRecordsByPage二次封装，如果需要获取多个字段的值列表，请优先使用getRecordsByPage
...
type IGetRecordsFilterInfo = {
    conjunction: FilterConjunction$1;
    /** 如果为IGetRecordsFilterInfo[]，则为筛选条件分组 */
    conditions: FilterInfoCondition$1[] | IGetRecordsFilterInfo[];
};
type IGetRecordsFilterInfoForMatted = {
    conjunction: FilterConjunction$1;
    /** 如果为IGetRecordsFilterInfo，则递归解析，支持筛选条件分组 */
    conditions: FilterInfoCondition$1[];
    children?: IGetRecordsFilterInfoForMatted[];
};
...
/** [tableId, viewId, baseToken] */
type ViewContext = [string, string, string | undefined];
/** IView 分页接口 */
...
interface IViewMessageModule$1 {
    ...
    /** @deprecated 使用table.getRecordIdListByPage + viewId 替代。获取记录id列表 */
    getVisibleRecordIdList(filterInfo?: IFilterInfo$1, sortInfo?: ISortInfo$1[]): Promise<(string | undefined)[]>;
    /** @deprecated 使用table.getRecordIdListByPage + viewId 替代。分页获取记录 ID 列表 */
...
/** [tableId] */
type TableContext = [string, string | undefined];
/** ITable 分页接口 api */
...
type IGetRecordListByPageParams = IGetRecordsByPageParams;
...
interface ITableMessageModule {
    ...
    /**
     * @deprecated 即将下线，建议迁移到 getRecordsByPage 接口，用法一致，此接口只能获取 200 条记录
     * @param param
     *  - @property pageSize: 获取数量，默认 200，最大不得超过 200
     *  - @property pageToken: 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果
     *  - @property filter: 过滤条件
    ...
    /**
     * @deprecated 即将下线，建议迁移到 getRecordIdListByPage 接口，以适配未来大数据量场景。这个接口现在最多仅会返回 200 条数据。
     * 获取表中所有记录 Id
     * @param filter 过滤条件
     * @param sort 排序条件
     * @returns
...
interface ITableApplicationModule {
    ...
    /**
     * @deprecated 即将下线，建议迁移到 getRecordListByPage 接口，以适配未来大数据量场景。这个接口现在最多仅会返回 200 条数据。
     */
    getRecordList: (filter?: Formula, sort?: Sort) => Promise<IRecordList>;
    /** 底层是 getRecordsByPage 接口，区别在于这个api返回的records是一个可迭代对象 */
    getRecordListByPage: (params: IGetRecordsByPageParams) => Promise<IGetRecordListByPageResponse>;
...
interface IUtil {
    isMobileLark: () => boolean;
    getLarkVersion: () => string;
...
interface IWorkspaceApplicationModule {
    /** 获取 bitable 上下文 */
    getBitable(baseToken: string): Promise<IBitableApp | null>;
}
interface IGetBaseListParams {
    query?: string;
    page?: {
        cursor?: string;
    };
}
interface IBaseListResult {
    base_list: IBaseInfo[];
    page: {
        cursor: string;
        hasMore: boolean;
...
interface IWorkspaceMessageModule {
    /** 获取当前 base 下所有表元信息 */
    getBaseList(params: IGetBaseListParams): Promise<IBaseListResult>;
    /** 是否已拉取数据完毕 */
    getBitableReady(baseToken: string): Promise<boolean>;
}
type IWorkspace = IWorkspaceApplicationModule & IWorkspaceMessageModule;
...
declare enum OpenErrorCode {
    ...
    TableAppNoFAPermissionError = 10212003,
 ...
 * 表相关操作无FA权限错误
 */
declare class TableAppNoFAPermissionError extends OpenError {
    constructor();
}
...
type IBridgeModuleInner = unknown;
type IBridgeExtra = unknown;
type IBridge = IBridgeMessageModule & ICommonBridgeApplicationModule;
