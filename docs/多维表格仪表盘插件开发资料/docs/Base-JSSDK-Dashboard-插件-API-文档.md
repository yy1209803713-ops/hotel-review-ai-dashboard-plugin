...
- `Dashboard` 模块**仅用于仪表盘插件**，**其他点位的插件无法调用该模块的接口**。
- 仪表盘插件**可以调用 JSSDK 其他模块中的接口**，但极少部分接口的返回和其他点位有不一致的情况出现，具体请参考具体接口文档中的备注。
...
<image token="IFM5bhtS2ohI1ZxEoiXc5AELnIb" width="2738" height="616" align="center"/>
...
<callout emoji="💡" background-color="blue" border-color="light-blue">

仪表盘插件和官方图表一样存在**多个状态**，**你需要在插件代码中感知多个不同状态并做出相应的处理**。
</callout>
...
<image token="PgiKbZuuMowwTlxSjgXckhMWneg" width="2880" height="1800" align="center"/>

图表**首次添加**至仪表盘时，插件处于创建状态，创建状态下插件相关数据并未落库，因此无法调用 [getConfig](https%3A%2F%2Fbytedance.larkoffice.com%2Fdocx%2FIahqdOrCto2xAKxPV7Oc8gyenfg%23B0Kvdrloto7csKxKyV2cXWZIn7f) 和 [getData](https%3A%2F%2Fbytedance.larkoffice.com%2Fdocx%2FIahqdOrCto2xAKxPV7Oc8gyenfg%23VGerd9tl0obF3nxP8YDczwirnVf) 方法。<text bgcolor="light-blue">除了初始化逻辑存在差异，创建状态和配置状态逻辑基本一致</text>。

## 配置状态

<image token="P0UQbgwQfo0lgdxh5eJcObhlnIe" width="2880" height="1800" align="center"/>

<image token="MYyvbmcWxozpeXxZyUDcI44Un5c" width="2880" height="1800" align="center"/>

在配置状态下用户可以对右侧配置进行修改，**插件左侧图表需要跟随右侧配置的变更实时渲染**，渲染数据可通过 [getPreviewData](https%3A%2F%2Fbytedance.larkoffice.com%2Fdocx%2FIahqdOrCto2xAKxPV7Oc8gyenfg%23Mun3dJ3JtoLhVpxspi6cB3Ekn2b) 接口进行获取。

## 展示状态

<image token="WfwAbF0tYocAxrxrEXFcjMUinhe" width="2880" height="1800" align="center"/>

<image token="Kf4yb4gE1o1x9hx2Sxqcjononhb" width="2880" height="1800" align="center"/>

**展示状态下插件需要隐藏配置相关内容，仅渲染图表相关内容即可**。同时展示状态下插件的渲染数据需要通过 [getData](https%3A%2F%2Fbytedance.larkoffice.com%2Fdocx%2FIahqdOrCto2xAKxPV7Oc8gyenfg%23VGerd9tl0obF3nxP8YDczwirnVf) 接口进行获取，配置和展示这两个状态是互斥的。

## 全屏状态

<image token="EdP8bDQUEooAOBxqsYqcaIb5nZe" width="3456" height="2160" align="center"/>
...
当前插件所处的状态，具体请参考[仪表盘插件状态](https%3A%2F%2Fbytedance.larkoffice.com%2Fdocx%2FIahqdOrCto2xAKxPV7Oc8gyenfg%23AJKZda3tEoECdixfF0jc5lTvnac)中的描述。
```typescript
...
获取仪表盘插件配置，包含`数据源配置`和插件`自定义配置`，在创建模式(dashboard.state === DashboardState.Create时)下调用会抛出错误。
```typescript
...
保存配置，通常在插件处于**配置状态**且用户修改并保存配置后调用，**调用该接口会关闭配置弹窗进入展示状态。**
```typescript
...
在**展示状态****(禁止用于配置状态，否则可能会出现意想不到的bug)**获取计算结果，计算结果是一个通用的二维数组格式，开发者自行决定这一组数据的消费方式。在创建模式(dashboard.state === DashboardState.Create时)下调用会抛出错误，创建模式时请使用`getPreviewData`。
```typescript
...
<image token="EBKwbetNgoznA2xaN9XcPpo7n1f" width="1550" height="674" align="center"/>
...
<grid cols="2">
  ...
  <column width="36">
    <image token="EptZbBaSUomPK0xp1lNcIsKQneg" width="650" height="1128" align="center"/>

    <text color="gray">计算条件</text> {align="center"}
  </column>
  <column width="63">
    ```typescript
    // 数据结构
    ...
    <text color="gray">计算结果</text> {align="center"}
  </column>
...
<grid cols="2">
  ...
  <column width="50">
    <whiteboard token="SZgMwmxp8h0SrsbcoRMc9Wbfn3g" width="398" height="421"/>

    <text color="gray">计算数据，单元格中填充的是 text/value</text> {align="center"}
    
  </column>
  <column width="50">
    <image token="IRjXba3VWo4fl3xtRB1cRZnNnAr" width="974" height="1528" align="center"/>

    <text color="gray">渲染结果（以柱状图为例）</text> {align="center"}
  </column>
...
在**配置状态****(禁止用于展示状态，否则可能会出现意想不到的bug)**获取用于预览的计算结果，注意接口入参仅需传递数据源配置，不需要传入插件自定义配置。
```typescript
...
为了保障你的插件能够在**自动化发送**中展示，**请务必在插件渲染完成后调用该方法**。
</callout>
...
以上任意一个环节的变更，都会导致`onDataChange`触发。
